...
const RING: Record<Rider['status'], string> = {
  ...
  offline: 'ring-muted-foreground/60'
};
...
const LEFT_ACCENT: Record<Rider['status'], string> = {
  ...
  offline: 'before:bg-muted-foreground'
};
...
export function RiderStatusCard({ rider, zone, hoursToday, onClick }: RiderStatusCardProps) {
  return (
    <button
      ...
      className={`group relative text-left bg-white border border-border hover:border-primary/40 rounded-xl p-3 transition flex flex-col justify-between gap-2.5 shadow-2xs hover:shadow-sm h-full w-full overflow-hidden cursor-pointer before:absolute before:left-0 before:top-0 before:bottom-0 before:w-[3px] ${LEFT_ACCENT[rider.status]}`}
    >
      <div className="flex items-center gap-2.5">
        <div className="relative shrink-0">
          <img
            src={rider.avatar}
            alt=""
            className={`w-9 h-9 rounded-full bg-panel-bg ring-2 ${RING[rider.status]} ring-offset-1 ring-offset-white`}
          />
          <span
        ...
        <div className="min-w-0 flex-1">
          <div className="text-xs font-bold text-foreground truncate group-hover:text-primary transition">
            {rider.name}
          </div>
      ...
      <div className="flex items-center justify-between gap-1.5">
        <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-panel-bg text-foreground border border-border truncate max-w-[65%]">
