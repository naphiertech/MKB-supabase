...
export function exportXLSX(data: ReportData, filename: string) {
...
export function exportPDF(
  ...
  function drawPageChrome(pageNum: number) {
    ...
    doc.text('AttenRider', marginX, 38);
    ...
    doc.setTextColor(107, 98, 88);
    doc.text(
      `Generated by AttenRider · MKB Corporation · ${generatedDate}`,
      pageWidth / 2,
      footerY,
      { align: 'center' }
    );
...
export async function generateReport(
  ...
  const filename = `attenrider_${opts.template}_${opts.from}_${opts.to}`;
