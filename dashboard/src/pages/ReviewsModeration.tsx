  ...
  return (
    <div className="p-4 md:p-6 lg:p-7 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-2 rounded-xl border border-[#EFEAE2] shadow-xs">
        <div className="flex gap-1">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              activeTab === 'pending'
                ? 'bg-[#FFF1E0] text-[#db6c00] border border-[#db6c00]/25'
                : 'text-[#6B6258] hover:text-[#1A1410] hover:bg-[#FAFAF7] border border-transparent'
            }`}
          >
            <Clock className="w-4 h-4" />
            Pending Verification
            {pendingReviews.length > 0 && (
              <span className="ml-1 bg-[#db6c00] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">
          ...
          >
            ...
            {approvedReviews.length > 0 && (
              <span className="ml-1 bg-[#6B6258] text-white text-[11px] font-bold px-2 py-0.5 rounded-full">
        ...
        <div className="text-xs text-[#6B6258] font-mono pr-2">
      ...
      {/* Loading state */}
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 bg-white border border-[#EFEAE2] rounded-2xl space-y-4">
          <div className="w-8 h-8 border-4 border-[#db6c00]/30 border-t-[#db6c00] rounded-full animate-spin" />
          <p className="text-sm text-[#6B6258] font-medium">Fetching reviews...</p>
        </div>
      ) : displayedReviews.length === 0 ? (
        /* Empty State */
        <div className="flex flex-col items-center justify-center py-16 bg-white border border-[#EFEAE2] rounded-2xl text-center px-4">
          <div className="w-14 h-14 bg-[#FAFAF7] rounded-full flex items-center justify-center border border-[#EFEAE2] text-[#6B6258] mb-4">
          ...
          <p className="text-sm text-[#6B6258] max-w-sm mt-1">
            {activeTab === 'pending'
          ...
          {displayedReviews.map((review) => (
            <div
              ...
              className={`bg-white border p-5 rounded-2xl shadow-sm flex flex-col justify-between transition-all duration-200 ${
                review.status === 'pending' ? 'border-[#EFEAE2] hover:border-[#db6c00]/30' : 'border-[#EFEAE2] hover:border-[#6B6258]/30'
              }`}
            >
              <div>
                ...
                <div className="flex justify-between items-start gap-4 mb-3">
                  <div>
                    <h3 className="font-semibold text-[#1A1410] text-base leading-snug">{review.name}</h3>
                    {review.role_title ? (
                      <p className="text-xs text-[#6B6258] font-medium mt-0.5">{review.role_title}</p>
                    ) : (
                      <p className="text-xs text-[#6B6258]/60 italic mt-0.5">Verified Customer</p>
                    )}
                  </div>
                  <span className="text-[11px] text-[#6B6258] font-mono bg-[#FAFAF7] border border-[#EFEAE2] px-2 py-0.5 rounded-md">
                ...
                <div className="flex gap-0.5 mb-3">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${
                        i < review.rating ? 'fill-[#db6c00] text-[#db6c00]' : 'text-gray-200'
                      }`}
                    />
                  ))}
                </div>
                ...
                {/* Comment */}
                <blockquote className="text-sm text-[#3E3830] leading-relaxed italic bg-[#FAFAF7]/50 border-l-2 border-[#db6c00]/20 pl-3 py-1 mb-4">
                  "{review.comment}"
                </blockquote>
              </div>
