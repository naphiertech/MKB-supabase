...
vi.mock('../attendance/attendanceService', () => ({ getAttendanceLogs: mocks.getAttendanceLogs }));
vi.mock('../monitoring/monitoringService', () => ({ getViolationsForReport: mocks.getViolationsForReport }));
vi.mock('../attendance/historicalAttendanceContext', () => ({ enrichAttendanceWithHistoricalZones: (logs: unknown[]) => Promise.resolve(logs) }));
