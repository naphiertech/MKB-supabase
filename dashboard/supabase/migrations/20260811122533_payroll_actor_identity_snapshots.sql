-- Preserve immutable, server-trusted actor identity snapshots for future
-- payroll workflow transitions. Existing rows intentionally remain null.

alter table public.payroll_records
  add column submitted_by_name_snapshot text,
  add column submitted_by_email_snapshot text,
  add column approved_by_name_snapshot text,
  add column approved_by_email_snapshot text,
  add column rejected_by_name_snapshot text,
  add column rejected_by_email_snapshot text,
  add column returned_by uuid,
  add column returned_by_name_snapshot text,
  add column returned_by_email_snapshot text,
  add column returned_at timestamptz,
  add column paid_by_name_snapshot text,
  add column paid_by_email_snapshot text;

alter table public.payroll_records
  add constraint payroll_records_returned_by_fkey
  foreign key (returned_by) references public.users(id) on delete restrict;

comment on column public.payroll_records.submitted_by_email_snapshot is
  'Confirmed Auth email captured when the payroll was submitted. Null on legacy rows.';
comment on column public.payroll_records.approved_by_email_snapshot is
  'Confirmed Auth email captured when the payroll was approved. Null on legacy rows.';
comment on column public.payroll_records.rejected_by_email_snapshot is
  'Confirmed Auth email captured when the payroll was rejected. Null on legacy rows.';
comment on column public.payroll_records.returned_by_email_snapshot is
  'Confirmed Auth email captured when the payroll was returned for revision. Null on legacy rows.';
comment on column public.payroll_records.paid_by_email_snapshot is
  'Confirmed Auth email captured when the payroll was marked Paid. Null on legacy rows.';

-- Approval and payment remain atomic, idempotent, row-locked transitions. The
-- actor identity is resolved once before the batch update, so every row and
-- audit entry receives the same trusted snapshot.
...
-- Direct submit/reject/return updates are still authorized by the existing
-- state machine, but actor UUIDs, snapshots, and timestamps are overwritten
-- from the authenticated server identity. Approval/payment remain RPC-only.
...
for each row execute function public.enforce_payroll_workflow_constraints();
